...
type FeatureLayers = {
  ...
  places: GeoJSON<PlaceGeojson['properties']> | null;
  relatedPlaces: GeoJSON<RelatedPlaceGeojson['properties']> | null;
};
...
const props = defineProps<{
  ...
  places: Array<PlaceGeojson>;
  relatedPlaces: Array<RelatedPlaceGeojson>;
}>();
...
function createPlaceTooltipContent(feature: PlaceGeojson | RelatedPlaceGeojson) {
  const label = feature.properties.name;
  const ancientLabel = feature.properties.name_antik;
  const type = feature.properties.art?.label;

  return `<div class="geo-map-tooltip">
    <strong>${label} (${type})</strong>
    <div>Ancient name: ${ancientLabel}</div>
  </div>`;
}
...
onMounted(() => {
  ...
  geoMap.featureGroups.places = geoJSON<PlaceGeojson['properties']>(undefined, {
    onEachFeature(feature: PlaceGeojson, layer: CircleMarker) {
      layer.bindTooltip(createPlaceTooltipContent(feature), {
        permanent: false,
        sticky: true,
      });
    },
    pointToLayer(feature, latlng) {
      const color = 'hsl(20deg 75% 75%)';
  ...
  geoMap.featureGroups.relatedPlaces = geoJSON<RelatedPlaceGeojson['properties']>(undefined, {
    onEachFeature(feature: RelatedPlaceGeojson, layer: CircleMarker) {
      layer.bindTooltip(createPlaceTooltipContent(feature), {
        permanent: false,
        sticky: true,
      });
    ...
    nextTick(() => {
      ...
      Object.values(layers).forEach(async (layer) => {
        if (visibleLayers.has(layer.id)) {
          geoMap.layers[layer.id] = geoJSON(await layer.geojson(), {
            // TODO:
          });
        }
      });
...
watch(
  [() => props.places, () => visibleFeatureGroups.value.has('places')],
  ([places, isVisible]) => {
    /** Update on next tick to ensure feature group is initialised. */
    nextTick(() => {
      const featureGroup = geoMap.featureGroups.places;
...
watch(
  [() => props.relatedPlaces, () => visibleFeatureGroups.value.has('relatedPlaces')],
  ([relatedPlaces, isVisible]) => {
    /** Update on next tick to ensure feature group is initialised. */
    nextTick(() => {
      const featureGroup = geoMap.featureGroups.relatedPlaces;

      featureGroup?.clearLayers();

      if (isVisible) {
        relatedPlaces.forEach((point) => {
          featureGroup?.addData(point);
        });
      }
    });
...
watch(
  ...
  () => {
    nextTick(() => {
      ...
      geoMap.featureGroups.places?.bringToFront();
      geoMap.featureGroups.relatedPlaces?.bringToFront();
...
watch(
  [() => props.areas, () => props.coneOrigins, () => props.relatedPlaces, () => props.places],
  ([areas, coneOrigins, relatedPlaces, places]) => {
    if ([areas, coneOrigins, relatedPlaces, places].every((features) => features.length === 0)) {
      const bounds = initialViewState.bounds;

      nextTick(() => {
        geoMap?.map?.flyToBounds(bounds, { duration: 0.25 });
      });
    } else if (areas.length === 0 && places.length === 1) {
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
      const place = places[0]!;

      nextTick(() => {
        const [lng, lat] = place.geometry.coordinates as [number, number];
        geoMap.map?.setView([lat, lng]);
      });
    } else {
