...
export default {
  ...
  methods: {
    fetchEntities(tabIndex) {
      ...
      const address = `${import.meta.env.VITE_APP_MMP_API_BASE_URL}/api/${
        this.tabs[tabIndex].api
      }/?${names[tabIndex]}=${this.tabs[tabIndex].filter}&${
        names[tabIndex]
      }_lookup=icontains&offset=${this.tabs[tabIndex].pagination.offset}&limit=${
